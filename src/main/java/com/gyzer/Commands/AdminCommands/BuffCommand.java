...
import com.gyzer.API.Events.GuildBuffLevelupEvent;
        ...
        switch (aot.toLowerCase()) {
            case "add": {
                ...
                if (add > 0) {
                    ...
                    MsgUtils.sendGuildMessage(guild.getMembers(), lang.plugin + lang.buff_levelup.replace("%target", buff.getDisplay()).replace("%value%", (current + add) + ""));
                }

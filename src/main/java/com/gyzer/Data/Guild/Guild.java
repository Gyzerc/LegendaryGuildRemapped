...
public class Guild {
    ...
    public List<String> getIntro() {
        ...
        if (i.size() < legendaryGuild.getConfigManager().DESC_MAX_LENGTH) {
            return i;
        }
        return new ArrayList<>(i.subList(0,legendaryGuild.getConfigManager().DESC_MAX_LENGTH));
    }

    public List<String> getNotice() {
        List<String> l = (notice != null ? notice : new ArrayList<>());
        if (l.size() < legendaryGuild.getConfigManager().NOTICE_MAX_LENGTH) {
            return l;
        }
        return new ArrayList<>(l.subList(0,legendaryGuild.getConfigManager().NOTICE_MAX_LENGTH));
    }
    ...
    public GuildTeamShopData getGuildTeamShopData() {
        return legendaryGuild.getTeamShopManager().getGuildTeamShopData(guild);
    }
    public LinkedList<String> getMembers() {
        return members != null ? members : (LinkedList<String>) Arrays.asList(owner);
    }
    public int getMaxMembers() {
        int defaultAmount =legendaryGuild.getConfigManager().MEMBERS.get(level);
    ...
    public void update(){
        legendaryGuild.getGuildsManager().updateGuild(this,false);
    ...
    public void delete(){
        legendaryGuild.getGuildsManager().removeGuildCache(guild);
        legendaryGuild.getDatabaseManager().delGuild(guild);
        Player p = Iterables.getFirst(Bukkit.getOnlinePlayers(),null);
        if (p != null) {
            new NetWorkMessageBuilder().setMessageType(NetWorkMessageBuilder.MessageType.Forward)
    ...
    public static class GuildHomeLocation {
        ...
        public Optional<Location> getLocation(){
            ...
            LegendaryGuild.getLegendaryGuild().info("该服务器不存在世界："+world, Level.SEVERE);
            return Optional.empty();
        }
    }

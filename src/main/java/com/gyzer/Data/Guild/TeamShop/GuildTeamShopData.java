package com.gyzer.Data.Guild.TeamShop;
...
public class GuildTeamShopData {
    ...
    public void update(boolean removeIfCached) {
        LegendaryGuild.getLegendaryGuild().getTeamShopManager().update(this,removeIfCached);
    ...
    public void randomShop() {
        List<String> ids = LegendaryGuild.getLegendaryGuild().getTeamShopManager().getShopItems();
        if (ids.size() > 0) {
            int roll = new Random().nextInt(ids.size());
            this.todayShopId = ids.get(roll);
            TeamShopItem item = LegendaryGuild.getLegendaryGuild().getTeamShopManager().getShopItem(todayShopId);
    ...
    public void bargain(Guild guild, String player) {
        if (!bargains.containsKey(player)) {
            ...
            TeamShopItem shopItem = LegendaryGuild.getLegendaryGuild().getTeamShopManager().getShopItem(todayShopId);
            ...
            if (currentPrice > min) {
                switch (LegendaryGuild.getLegendaryGuild().getTeamShopManager().getTeamShopConfigManager().bargainMode) {
                    case RANDOM: {
                        //当可砍价
                        if (canBargained > 0) {
